...
import work.nep.config.KafkaProperties;

class Tr33T33KafkaResourceIT {
    ...
    void setup() {
        ...
        Tr33T33KafkaResource kafkaResource = new Tr33T33KafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/tr-33-t-33-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/tr-33-t-33-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
